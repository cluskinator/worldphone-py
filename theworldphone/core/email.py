...
def _override_html():
    ...
    It's too bad `tbtools.Traceback` doesn't copy module constants to instance variables where
    ...
    # Get new HTML.
    email_template = current_app.jinja_env.get_template('macros/exception_email.html')
...
def send_exception(subject):
    ...
    This function must be called while the exception is happening.
    It picks up the raised exception with sys.exc_info().
    ...
    subject = '[FRLA.mobile CMS] Application Error: {}'.format(subject)

    # Send email.
    mail.send_email(
        subject=subject,
        to=[{'email%s' % idx: email for idx, email in enumerate(current_app.config['ADMINS'])}],
        html=html)
